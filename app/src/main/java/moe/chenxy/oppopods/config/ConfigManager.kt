...
data class AppConfig(
    ...
    val persistentIsland: Boolean = ConfigManager.DEFAULT_PERSISTENT_ISLAND,
...
object ConfigManager {
    ...
    const val PREF_KEY_PERSISTENT_ISLAND = "persistent_island"
    ...
    const val DEFAULT_PERSISTENT_ISLAND = true
    ...
    fun persistentIsland(): Boolean = current().persistentIsland
    ...
    fun updatePersistentIsland(prefs: SharedPreferences, service: XposedService?, enabled: Boolean) {
        val config = current().copy(persistentIsland = enabled)
        save(prefs, service, config)
    }
    ...
    private fun writePrefs(prefs: SharedPreferences, config: AppConfig) {
        prefs.edit()
            ...
            .putBoolean(PREF_KEY_PERSISTENT_ISLAND, config.persistentIsland)
    ...
    private fun readConfig(prefs: SharedPreferences, source: String): AppConfig {
        ...
        val directPersistentIsland = if (prefs.contains(PREF_KEY_PERSISTENT_ISLAND)) {
            prefs.getBoolean(PREF_KEY_PERSISTENT_ISLAND, DEFAULT_PERSISTENT_ISLAND)
        } else {
            null
        }
        ...
        if (!directFakeDeviceId.isNullOrBlank()) {
            return config.copy(
                ...
                persistentIsland = directPersistentIsland ?: config.persistentIsland,
    ...
    private fun changedFields(oldConfig: AppConfig, newConfig: AppConfig): List<String> {
        return buildList {
            ...
            if (oldConfig.persistentIsland != newConfig.persistentIsland) {
                add("persistentIsland=${oldConfig.persistentIsland}->${newConfig.persistentIsland}")
            }
